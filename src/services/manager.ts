...
export async function getTeamGoalSheets(
  cycleId = "q3_2026",
): Promise<(GoalSheet & { employeeName: string })[]> {
  return apiRequest<(GoalSheet & { employeeName: string })[]>(
    `/api/goals/team?cycle_id=${cycleId}`,
  );
}
...
export async function rejectSheet(
  ...
  return apiRequest<{ success: boolean }>(`/api/goals/${sheetId}/reject`, "POST", {
