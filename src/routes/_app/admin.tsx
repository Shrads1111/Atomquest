  ...
  head: () => ({
    meta: [
      { title: "Admin Control — GoalSync" },
      {
        name: "description",
        content:
          "Corporate governance dashboard with cycle lifecycle controls and immutable audit logs.",
      },
    ],
  }),
  component: AdminPage,
...
function AdminPage() {
  ...
  useEffect(() => {
    ...
    const qAudits = query(collection(db, "audit-logs"), orderBy("ts", "desc"), limit(40));
    ...
    const qCycles = query(collection(db, "cycles"));
        ...
        if (!snapshot.empty) {
          ...
          if (currentCycle.currentStage) {
            const formattedStage =
              currentCycle.currentStage.charAt(0).toUpperCase() +
              currentCycle.currentStage.slice(1);
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[50vh] space-y-4">
        ...
        <div className="text-sm font-mono-metric text-indigo-200">
          Syncing Global Enterprise Control Hub...
        </div>
      </div>
    );
  }
  ...
  const pendingApprovalsCount = sheetsList.filter(
    ...
    <div className="space-y-8">
      ...
      {!isOnline && (
        <div className="p-3 bg-amber-500/10 border border-amber-500/20 text-amber-300 text-xs rounded-lg font-mono-metric flex items-center gap-1.5 animate-pulse">
          <AlertTriangle className="h-4 w-4" /> You are operating in offline mode. Central
          monitoring will sync when online.
        </div>
      )}
      ...
      <GlassCard className="p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 relative">
          {/* Phase 1 */}
          <div
            className={`rounded-xl border p-5 transition-all flex flex-col justify-between ${activePhase === "Setup" ? "border-blue-500 bg-blue-500/5 shadow-[0_0_30px_rgba(37,99,235,0.25)]" : "border-emerald-500/30 bg-emerald-500/5"}`}
          >
            ...
            <div className="text-xs text-muted-foreground font-mono-metric mt-1">
              Drafting of goals sheets enabled
            </div>
            <button
          ...
          {/* Phase 2 */}
          <div
            className={`rounded-xl border p-5 transition-all flex flex-col justify-between ${activePhase === "Tracking" ? "border-blue-500 bg-blue-500/5 shadow-[0_0_30px_rgba(37,99,235,0.25)]" : "border-white/8 bg-white/2"}`}
          >
            <div className="flex items-center justify-between">
              <div className="text-[10px] uppercase tracking-[0.2em] text-muted-foreground font-mono-metric">
                Phase 2
          ...
          {/* Phase 3 */}
          <div
            className={`rounded-xl border p-5 transition-all flex flex-col justify-between ${activePhase === "Evaluation" ? "border-blue-500 bg-blue-500/5 shadow-[0_0_30px_rgba(37,99,235,0.25)]" : "border-white/8 bg-white/2"}`}
          >
            <div className="flex items-center justify-between">
              <div className="text-[10px] uppercase tracking-[0.2em] text-muted-foreground font-mono-metric">
                Phase 3
            ...
            <div className="text-xs text-muted-foreground font-mono-metric mt-1">
              Final supervisor rating window
            </div>
            <button
              disabled={processingCycle}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-8 gap-4">
        ...
        <MetricTile
          ...
          value={String(sheetsList.filter((s) => s.status === "Approved").length)}
          sub="Locked objectives"
          accent="emerald"
        />
        <MetricTile
        ...
        <MetricTile
          ...
          value="88.5%"
          sub="Compliance level"
          accent="emerald"
        />
        <MetricTile
          label="Compliance rate"
          value={`${lockCompliancePercent}%`}
          sub="Compliance status"
          accent="crimson"
        />
        ...
        <GlassCard className="p-5 flex flex-col justify-between">
          ...
          <div className="flex flex-col gap-3.5 justify-center py-1">
            {(
              ...
              <div
                key={i}
                className="flex items-center gap-3 justify-between text-xs font-mono-metric"
              >
          ...
          <div className="flex justify-center gap-3 text-[9px] text-muted-foreground font-mono-metric pt-3 border-t border-white/5">
            <span className="flex items-center gap-1">
              <span className="w-2.5 h-2.5 rounded bg-emerald-500"></span> 100%
            </span>
            <span className="flex items-center gap-1">
              <span className="w-2.5 h-2.5 rounded bg-indigo-500"></span> ≥60%
            </span>
            <span className="flex items-center gap-1">
              <span className="w-2.5 h-2.5 rounded bg-amber-500"></span> &lt;60%
            </span>
            <span className="flex items-center gap-1">
              <span className="w-2.5 h-2.5 rounded bg-slate-700"></span> 0%
            </span>
          </div>
        </GlassCard>
      ...
      <GlassCard className="overflow-hidden">
        <div className="px-5 py-4 border-b border-white/5 flex items-center justify-between flex-wrap gap-4">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5 font-mono-metric">
              Audit global sheet locks, check validation progress, and override active states.
            </p>
          </div>
        </div>
              ...
              {sheetsList.map((u, idx) => (
                <tr key={u.id || idx} className="row-hover border-t border-white/5 transition-all">
                  <td className="px-5 py-4 font-semibold text-white/95">
                    {u.employeeName || "Core Developer"}
                  </td>
                  <td className="px-5 py-4 text-xs text-muted-foreground font-mono-metric">
                    {u.cycleId}
                  </td>
                  <td className="px-5 py-4 text-indigo-300 text-xs">
                    {u.lockStatus || "Unlocked"}
                  </td>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <GlassCard className="p-6">
          ...
          <p className="text-xs text-muted-foreground mt-0.5 mb-5 font-mono-metric">
            SLA compliance scan execution logs. Mitigate active violations instantly.
          </p>

          <div className="text-center py-4 border border-dashed border-white/10 rounded-xl space-y-3">
            <AlertTriangle className="h-8 w-8 text-rose-400 mx-auto" />
            <div className="text-xs font-mono-metric text-white/95">
              Trigger scan to check overdue goals check-ins across Firestore
            </div>
            <button
              disabled={processingCycle}
        ...
        <GlassCard className="p-6 flex flex-col justify-between">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5 mb-5 font-mono-metric">
              Obsidian Cinematic transaction monitors. All metrics cryptographically aligned.
            </p>
            ...
            <div className="grid grid-cols-2 gap-3 text-xs font-mono-metric mb-4">
              ...
              <div className="glass-elevated p-3 text-center">
                ...
                <span className="text-emerald-400 text-xl font-bold mt-1 block">
                  SOC2 Compliant
                </span>
              </div>
            </div>
      ...
      <GlassCard className="overflow-hidden">
        <div className="px-5 py-4 border-b border-white/5 flex items-center justify-between flex-wrap gap-2">
          <div>
            <div className="text-[10px] uppercase tracking-[0.22em] text-indigo-300 font-mono-metric">
              Immutable Audit Interlock Logs
            </div>
            <h3 className="text-lg font-bold mt-0.5">System Modification Audit Logs</h3>
            <p className="text-xs text-muted-foreground mt-0.5 font-mono-metric">
              Verification audits detailing administrator actions, goal adjustments, and
              force-unlock triggers.
            </p>
          </div>
          <button className="btn-ghost" onClick={handleExportAudits}>
            ...
            <tbody>
              {auditLogs.map((l, i) => (
                <tr key={i} className="row-hover border-t border-white/5">
                  ...
                  <td className="px-5 py-3.5 text-indigo-200 font-semibold">
                    {l.actorName || l.actorId}
                  </td>
                  ...
                  <td
                    className="px-5 py-3.5 text-muted-foreground max-w-sm truncate"
                    title={l.rationale}
                  >
                    {l.rationale}
                  </td>
