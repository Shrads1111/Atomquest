...
export const Route = createFileRoute("/_app/goals/new")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Multi-step goal registration wizard with live weight validation.",
      },
    ],
  }),
...
function WizardPage() {
  ...
  const [goals, setGoals] = useState<GoalNode[]>([
    {
      id: "n1",
      title: "Implement Core Platform Optimization Modules",
    ...
    {
      id: "n2",
      title: "Reduce Bundle Size Across Edge Runtime",
      thrustArea: "Operational Efficiency Optimization",
      uom: "numeric",
      assignedWeightageFactor: 0.3,
    },
    {
      id: "n3",
      title: "Launch Internal Developer Telemetry SDK",
          ...
          {[
            ...
            <div key={s.n} className="flex items-center gap-3 shrink-0">
              ...
              <div className={s.state === "active" ? "text-white" : "text-muted-foreground"}>
                <div className="text-xs uppercase tracking-[0.16em] font-mono-metric">
                  Step {s.n}
                </div>
            ...
            <GlassCard key={g.id} className="p-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="sm:col-span-1">
                  ...
                  >
                    {THRUST_AREAS.map((t) => (
                      <option key={t} value={t} className="bg-[#0C0E17]">
                        {t}
                      </option>
                    ))}
                  </select>
                  ...
                  {UOM.map((u) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="text-[11px] text-muted-foreground font-mono-metric mt-0.5">
                          {u.desc}
                        </div>
                      </button>
                    );
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="font-mono-metric text-sm">
                    <span className="text-indigo-300">
                      {(g.assignedWeightageFactor * 100).toFixed(0)}%
                    </span>
                  </div>
                </div>
                <input
        ...
        <div className="space-y-4 lg:sticky lg:top-24 lg:self-start">
          ...
          <GlassCard className="p-5">
            ...
            <ul className="space-y-2 text-sm">
              <CheckItem
                ok={goals.length <= 8}
                label={`Goals within bounds (${goals.length} / 8)`}
              />
              <CheckItem
                ok={goals.every((g) => g.assignedWeightageFactor >= 0.1)}
                label="Each goal ≥ 10% weight floor"
              />
              <CheckItem
                ok={Math.abs(validation.totalWeightPct - 100) < 0.05}
                label="Total weight = 100%"
              />
              <CheckItem
                ok={goals.every((g) => g.title.trim().length > 0)}
                label="All titles populated"
              />
            </ul>
          </GlassCard>
...
function Label({ children }: { children: React.ReactNode }) {
  return (
    <label className="text-[10px] uppercase tracking-[0.2em] text-muted-foreground font-mono-metric">
      {children}
    </label>
  );
}
